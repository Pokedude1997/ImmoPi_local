...
export const Dashboard = () => {
  ...
  const processRecurringPayments = (recurring: RecurringPayment[], currentYear: number) => {
    ...
    recurring.filter(r => r.active).forEach(r => {
      ...
      while (nextDue <= today && nextDue.getFullYear() === currentYear) {
        const date = nextDue.toISOString().split('T')[0];
        ...
        api.createTransaction(backendPayload)
          .catch(err => console.error('Failed to create recurring transaction:', err));
      ...
      const updateBackendPayload = {
        ...r,
        isActive: r.active ? 1 : 0,
        property_id: r.propertyId,
        category_id: r.categoryId,
        counterparty_id: r.counterpartyId
      };
      delete updateBackendPayload.active;
      delete updateBackendPayload.propertyId;
      delete updateBackendPayload.categoryId;
      delete updateBackendPayload.counterpartyId;
  ...
  const processMortgageTransactions = (property: Property, currentYear: number, categories: Category[]) => {
    ...
    while (current <= yearEnd) {
      ...
      if (paymentDate.getFullYear() === currentYear && paymentDate <= today) {
        ...
        // Check if THIS PROPERTY already has mortgage transactions for THIS MONTH/Year
        // This prevents duplicates even if payment date or category changes
        const monthKey = `${property.id}-${paymentDate.getFullYear()}-${paymentDate.getMonth()}`;
        
        // Find all mortgage transactions for this property and month
        const mortgageTransactionsThisMonth = transactions.filter(tx =>
          ...
          new Date(tx.date).getFullYear() === paymentDate.getFullYear() &&
          new Date(tx.date).getMonth() === paymentDate.getMonth()
        );
