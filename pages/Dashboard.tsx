  ...
  const processMortgageTransactions = (property: Property, currentYear: number, categories: Category[]) => {
    ...
    while (current <= yearEnd) {
      ...
      if (paymentDate.getFullYear() === currentYear && paymentDate <= today) {
        ...
        // Check if interest transaction already exists
        const interestExists = transactions.some((tx) =>
          tx.propertyId === property.id &&
          tx.categoryId === interestCategoryId &&
          tx.isAutoGenerated &&
          tx.date === date &&
          tx.description?.includes('Mortgage interest')
        );

        // Check if principal transaction already exists
        const principalExists = transactions.some((tx) =>
          tx.propertyId === property.id &&
          tx.categoryId === principalCategoryId &&
          tx.isAutoGenerated &&
          tx.date === date &&
