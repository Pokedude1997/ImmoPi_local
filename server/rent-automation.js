 ...
 */
async function createRentTransaction({ contract, date, warmRent, rentCategory, categories, isAutoGenerated = true }) {
 ...
 */
async function createRentPaymentAndTransaction({ contract, date, warmRent, rentCategory, categories, status = 'PENDING', isAutoGenerated = true }) {
  // Create transaction
...
async function processTenantContract(contract, today, existingPayments, categories) {
  ...
  try {
    ...
    // Process payments from first payment date, covering past dates and up to 12 months into the future
