  ...
  return new Promise((resolve, reject) => {
    db.run(
      `INSERT INTO transactions (
        ...
        counterparty_id, document_id, isAutoGenerated, source
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        tx.date, 
        tx.amount, 
  ...
  return new Promise((resolve, reject) => {
    db.run(
      `INSERT INTO rent_payments (
        ...
        status, payment_method, transaction_id, notes, source
      ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
...
async function createRentPaymentAndTransaction({ contract, date, warmRent, rentCategory, categories, status = 'PENDING', isAutoGenerated = true, source = null }) {
  ...
  // Create rent payment linked to transaction with source tag
